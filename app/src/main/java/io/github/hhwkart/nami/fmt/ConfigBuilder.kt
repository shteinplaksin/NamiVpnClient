...
import io.github.hhwkart.nami.bg.ProxyAuth
...
import io.github.hhwkart.nami.database.ProxyEntity.Companion.TYPE_CONFIG
import io.github.hhwkart.nami.database.SagerDatabase
import io.github.hhwkart.nami.fmt.ConfigBuildResult.IndexEntity
import io.github.hhwkart.nami.fmt.hysteria.HysteriaBean
import io.github.hhwkart.nami.fmt.hysteria.buildSingBoxOutboundHysteriaBean
import io.github.hhwkart.nami.fmt.internal.ChainBean
import io.github.hhwkart.nami.fmt.shadowsocks.ShadowsocksBean
import io.github.hhwkart.nami.fmt.shadowsocks.buildSingBoxOutboundShadowsocksBean
import io.github.hhwkart.nami.fmt.socks.SOCKSBean
import io.github.hhwkart.nami.fmt.socks.buildSingBoxOutboundSocksBean
import io.github.hhwkart.nami.fmt.ssh.SSHBean
import io.github.hhwkart.nami.fmt.ssh.buildSingBoxOutboundSSHBean
import io.github.hhwkart.nami.fmt.tuic.TuicBean
import io.github.hhwkart.nami.fmt.tuic.buildSingBoxOutboundTuicBean
import io.github.hhwkart.nami.fmt.v2ray.StandardV2RayBean
import io.github.hhwkart.nami.fmt.v2ray.buildSingBoxOutboundStandardV2RayBean
import io.github.hhwkart.nami.fmt.wireguard.WireGuardBean
import io.github.hhwkart.nami.fmt.wireguard.buildSingBoxOutboundWireguardBean
import io.github.hhwkart.nami.ktx.isIpAddress
import io.github.hhwkart.nami.ktx.mkPort
import io.github.hhwkart.nami.utils.PackageCache
import io.github.hhwkart.nami.core.*
...
import io.github.hhwkart.nami.core.proxy.anytls.buildSingBoxOutboundAnyTLSBean
...
import io.github.hhwkart.nami.core.proxy.shadowtls.buildSingBoxOutboundShadowTLSBean
...
fun buildConfig(
    ...
    if (proxy.type == TYPE_CONFIG) {
        ...
        if (bean.type == 0) {
            val rawConfig = RawConfigLanPolicy.configForBuild(bean.config, forExport) {
                SagerNet.application.hasLocalNetworkPermission()
            }
    ...
    val canExposeToLan = LocalNetworkPermissionPolicy.shouldExposeToLan(
        allowAccess = DataStore.allowAccess,
        sdkInt = Build.VERSION.SDK_INT,
        hasPermission = if (forExport || forTest) {
            false
        } else {
            SagerNet.application.hasLocalNetworkPermission()
        },
    ...
    val bind = if (canExposeToLan) "0.0.0.0" else LOCALHOST
    ...
    return MyOptions().apply {
        ...
        if (!forTest) {
            ...
            inbounds.add(Inbound_MixedOptions().apply {
                ...
                sniff = needSniff
                sniff_override_destination = needSniffOverride
                // R1-R4: local inbound auth. Fail closed — never start an
                // unauthenticated local proxy; if credentials are missing the
                // exception aborts service startup. Auth is mandatory when the
                // setting is on (default) or the inbound is exposed to LAN.
                val authRequired = DataStore.socks5Auth || DataStore.allowAccess
                if (authRequired) {
                    if (!ProxyAuth.valid) {
                        throw IllegalStateException("local proxy credentials not available")
                    }
                    users = mutableListOf(SingBoxOptions.User().apply {
        ...
        // Keep this rule after persisted user rules: an existing domain rule,
        // including a blocking rule, must retain first-match priority. The
        // matching DNS direct rule is appended in the same user-rule phase.
        if (!forTest && isVPN && DataStore.websiteBypassEnabled) {
            val websiteBypassDecision = WebsiteBypassPolicy.safeRuntimeHosts(
                hosts = DataStore.websiteBypassDomains.orEmpty().split('\n', ',', '\r'),
                rules = extraRules.map {
            ...
            websiteBypassDecision.rejected.forEach { rejected ->
                Logs.w("Website Bypass entry was skipped: $rejected")
            }
            if (websiteBypassDecision.acceptedHosts.isNotEmpty()) {
                val domains = websiteBypassDecision.acceptedHosts.toMutableList()
                route.rules.add(Rule_DefaultOptions().apply {
                    domain_suffix = domains
                    outbound = TAG_BYPASS
                })
                userDNSRuleList += DNSRule_DefaultOptions().apply {
                    makeSingBoxRule(domains)
                    server = "dns-direct"
                }
            }
        }
