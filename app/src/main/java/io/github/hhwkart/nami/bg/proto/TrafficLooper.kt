...
import io.github.hhwkart.nami.fmt.TAG_BYPASS
import io.github.hhwkart.nami.fmt.TAG_PROXY
...
class TrafficLooper
    ...
    private suspend fun loop() {
        val configuredInterval = DataStore.speedInterval.toLong()
        val delayMs = if (configuredInterval <= 0L) 1000L else configuredInterval
        ...
        while (sc.isActive) {
            ...
            data.notification?.apply {
                if (listenPostSpeed && configuredInterval > 0L) postNotificationSpeedUpdate(speed)
            }
